    ...
    try:
        ...
        if total_calcs == 0:
            ...
            licenciements = await Dossier.find(
                Dossier.user_id == request_user.id, {"dispute_details.nature": "licenciement"}
            ).count()
            demissions = await Dossier.find(
                Dossier.user_id == request_user.id, {"dispute_details.nature": "demission"}
            ).count()
        ...
        return {"licenciements": licenciements_pct, "demissions": demissions_pct, "total": total}
    except Exception as e:
